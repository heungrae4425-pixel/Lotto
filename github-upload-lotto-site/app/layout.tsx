      ...
      <body>
        ...
        <header className="border-b border-slate-200 bg-white">
          <nav className="container-page flex flex-wrap items-center justify-between gap-3 py-4">
            <Link href="/" className="text-lg font-bold text-ink">로또 6/45 통계</Link>
            <div className="flex flex-wrap gap-3 text-sm font-medium text-muted">
              <Link href="/draws">회차 목록</Link>
              <Link href="/stats">통계</Link>
              <Link href="/generator">번호 생성기</Link>
              <Link href="/insights/frequent-numbers">많이 나온 번호</Link>
            </div>
          </nav>
        </header>
        <main>{children}</main>
        <footer className="mt-12 border-t border-slate-200 bg-white">
          <div className="container-page grid gap-5 py-8">
            <div className="flex flex-wrap gap-3 text-sm text-slate-600">
              ...
              <Link href="/generator">로또번호 생성기</Link>
            ...
            <Disclaimer />
            <p className="text-xs text-slate-500">복권 구매, 베팅, 유료 번호 판매 기능을 제공하지 않습니다.</p>
          </div>
        </footer>
      </body>
    </html>
